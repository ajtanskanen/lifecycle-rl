...
class LazyMemory(dict):
    ...
    def reset(self):
        ...
        self['action'] = np.empty((self.capacity, self.action_shape), dtype=np.int8)
    ...
    def sample(self, batch_size):
        if len(self)<1:
            print("error: cannot sample memory with len 0")
            return None
    ...
    def _sample(self, indices, batch_size):
        ...
        states = torch.FloatTensor(states).to(self.device)#.float()
        next_states = torch.FloatTensor(
            next_states).to(self.device)#.float()
...
class LazyMemory_multidiscrete(dict):

    def __init__(self, capacity, state_shape, actions, device):
        super(LazyMemory_multidiscrete, self).__init__()
        self.capacity = int(capacity)
        self.state_shape = state_shape
        self.action_shape = actions.shape
    ...
    def reset(self):
        self['state'] = []
        self['next_state'] = []
        self['action'] = np.empty((self.capacity, self.action_shape[0]), dtype=np.int8)
        self['reward'] = np.empty((self.capacity, 1), dtype=np.float32)
        self['done'] = np.empty((self.capacity, 1), dtype=np.float32)
    ...
    def _append(self, state, action, reward, next_state, done):
        ...
        #print('memory',action,self['action'][self._p])
...
class LazyMultiStepMemory_multidiscrete(LazyMemory_multidiscrete):

    def __init__(self, capacity, state_shape, action_shape, device, gamma=0.99,
                 multi_step=3):
        super(LazyMultiStepMemory_multidiscrete, self).__init__(
            capacity, state_shape, action_shape, device)
